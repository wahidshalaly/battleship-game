...
public class CreatePlayerCommandHandlerTests
{
    ...
    public async Task Handle_WhenValidUsername_ShouldCreatePlayerAndReturnResult()
    {
        ...
        A.CallTo(() => _repository.UsernameExistsAsync(username, _cancellationToken)).Returns(false);
        A.CallTo(() => _repository.SaveAsync(A<Player>._, _cancellationToken)).Returns(expectedPlayerId);
