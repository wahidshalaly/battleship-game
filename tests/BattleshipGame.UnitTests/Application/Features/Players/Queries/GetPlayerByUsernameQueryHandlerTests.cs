...
public class GetPlayerByUsernameQueryHandlerTests
{
    ...
    public async Task Handle_WhenPlayerExistsByUsername_ShouldReturnPlayerResult()
    {
        ...
        A.CallTo(() => _playerRepository.GetByUsernameAsync(username, ct)).Returns(player);
    ...
    public async Task Handle_ShouldMapAllPlayerPropertiesCorrectly()
    {
        ...
        A.CallTo(() => _playerRepository.GetByUsernameAsync(expectedUsername, ct)).Returns(player);
    ...
    [InlineData(10)]
    [InlineData(100)]
    public async Task Handle_WithDifferentGameHistoryCounts_ShouldReturnCorrectTotalGames(
        int historyCount
    )
    {
        // Arrange
    ...
    )
    {
        ...
        {
            var gameHistory = (System.Collections.Generic.List<GameId>)
                gameHistoryField.GetValue(player)!;
